...
import { AddAssetModal } from '@/src/components'
import { convertArrayToObject, handleAssetModel, handleAssetStatus } from '@/src/utils/formatters'
import { unstable_batchedUpdates } from 'react-dom'
import { CompanyDataType } from '@/src/@types/companies'
import { UnitDataType } from '@/src/@types/units'
import { AssetsDataType, FormattedAssetsDataType } from '@/src/@types/assets'

const AssetsComponent = () => {
  const [data, setData] = useState<Array<FormattedAssetsDataType>>()
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const formattedUnits = data.map((unit: AssetsDataType) => ({
        ...unit,
        companyName: companyNameArray[unit.companyId],
        unitName: unitNameArray[unit.unitId],
        model: handleAssetModel(unit?.model),
        status: handleAssetStatus(unit?.status)
      }))
  ...
  return (
    <>
      ...
      >
        Novo equipamento <PlusCircleFilled style={{ marginLeft: '6px' }} />
      </Typography>
